...
def Pre_Div(U,V,user_movie,movie_user,u,sigmaE2,sigmaU2,k,alpha):
	...
	for i in xrange(k):
		...
		for w in Theta:
			if movie_user.has_key(w) and len(movie_user[w])>10:
				p_g[w]=0
				for j in Omega:
					p_g[w]=p_g[w]+alpha*W(user_movie[u],j)*Max(0,np.dot(V[w,:],V[j,:])-maxInS(V,S,j))
		...
		w=0
		for j in Theta:
			if movie_user._key(j) and len(movie_user[j])>10:
				med=p_R[j]+p_g[j]
				if med>max:
					max=med
					w=indi
